...
is_uninstaller() {
  if [ "$ZIPNAME" = "uninstall" ]; then
    ui_print "- Uninstall Gmail Google"
    rm -rf $SYSTEM_ADDOND/70-gmail.sh
    rm -rf $SYSTEM_APP/Gmail
    # End installation
    on_installed
  fi
}
...
sdk_v25_install() {
  # Remove Gmail Google
  rm -rf $SYSTEM_APP/Gmail
  ui_print "- Installing Gmail Google"
  if [ "$BOOTMODE" = "false" ]; then
  ...
  tar -xf $ZIP_FILE/sys/Gmail.tar.xz -C $TMP_SYS
  pkg_TMPSys
...
df_reclaimed() {
  PKG="$(find /system -type d -iname '*Gmail*')"
