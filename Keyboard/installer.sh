...
is_uninstaller() {
  if [ "$ZIPNAME" = "uninstall" ]; then
    ...
    rm -rf $SYSTEM_ADDOND/70-keyboard.sh
    rm -rf $SYSTEM_APP/Keyboard
    # End installation
    on_installed
  fi
}
...
df_reclaimed() {
  PKG="$(find /system -type d -iname '*Keyboard*')"
  RES="$(find /system -type d -iname '*LatinIME*')"
