...
BITGAPPS="zip/core/Gearhead.tar.xz zip/Permissions.tar.xz"
...
is_uninstaller() {
  if [ "$ZIPNAME" = "uninstall" ]; then
    ...
    rm -rf $SYSTEM_ADDOND/70-gearhead.sh
    rm -rf $SYSTEM_PRIV_APP/Gearhead
    rm -rf $SYSTEM_ETC_PERM/gearhead.xml
    # End installation
    on_installed
  fi
}
...
df_reclaimed() {
  PKG="$(find /system -type d -iname '*Gearhead*')"
